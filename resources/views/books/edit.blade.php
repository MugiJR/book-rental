    ...
    <form action="/books/{{$book['id']}}" method="post">
        ...
        <div class="form-group d-flex flex-wrap py-3">
            ...
            <div class="custom-control custom-switch col-sm-2">
                <input type="checkbox" class="custom-control-input" name="genres[]" id="genre-{{ $genre['id'] }}" value="{{ $genre['id'] }}" @if ($book->genres->contains($genre)) checked @endif {{ (is_array(old('genres')) && in_array($genre['id'], old('genres'))) ? ' checked' : '' }}
                >
