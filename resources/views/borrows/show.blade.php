...
<div class="container py-3">
    <h3>My Rentals</h3>
    <h6>Book Data</h6>
    <h5>Title: <a href="{{route('books.show',['book'=>$borrow->book->id])}}">{{$borrow->book->title}}</h5></a>
    <p class="card-text">Authors: {{$borrow->book->author}}</p>
    <p class="card-text">Released at: {{$borrow->book->released_at}}</p>
    <h6>Rental data</h6>
    <p class="card-text">Borrower Name: {{$borrow->reader->name}}</p>
    <p class="card-text">Date of rental request: {{$borrow->request_process_at}}</p>
    ...
    @endif
    @if($borrow->deadline<now()) <button class="btn btn-warning" disabled>Deadline already crossed</button>
        @endif
        @can('is_librarian')
